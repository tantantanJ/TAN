...
public class resetPWD extends HttpServlet{
	...
	static Logger logger = Logger.getLogger(resetPWD.class.getName());
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {		request.setCharacterEncoding("utf-8");
	    ...
	    if(key == null || key.equals("") ){  
	    	...
	    	response.sendRedirect("success.html?active=noSuchCode");
	    	return ;  
	    } else {
	    	...
	    	sqlStr="SELECT ZhuCeYouXiang, XingMing, MiMaLingPai FROM webuser " +
...
//		    				"(RenYuanGongHao, RenYuanXingMing, RenYuanXingBie, TiYanGongHao, GangWeiXingZhi, GangWeiJiBie, KeShenPi, BuMenBiaoShi, CaoZuoMiMa, YunXuDengLu, ZhuoMianPeiZhi) " +
//		    				"VALUES ("+
//		    				"'"+renYuanGongHao+"', "+
//		    				"'"+(String) ((HashMap) listResults.get(0)).get("XingMing")+"', "+
//		    				"'男', "+"1, "+"'注册用户', "+"0, "+"1, "+"1, "+
//		    				"NULL, "+"1, "+
...
//	    			logger.info("resultInsert="+resultInsert);
	    			...
	    			response.setCharacterEncoding("utf-8");
				  	response.setStatus(302);
				  	response.sendRedirect("success.html?active=active");
	    		}else{
	    			logger.info("用户验证更新数据库失败。ZhuCeYouXiang= "+ ZhuCeYouXiang);
		    		response.setStatus(302);
		    		response.sendRedirect("success.html?active=activeError");
	    		}
